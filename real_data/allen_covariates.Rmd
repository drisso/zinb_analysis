...
plot(zifa_deseq, pch=19, col=colMerged, main="ZIFA FQ")
```

## ZINB

```{r zinb}
zinb <- zinbFit(raw, ncores = ncores, K = 2, epsilon = 1e5)
...
zinb_batch <- zinbFit(raw, ncores = ncores, K = 2, X=qcpca$x[,1:2], epsilon=1e3)
...
```{r write_out, dependson=c("zinb", "zifa", "pca")}
save(zinb, zinb_batch, zifa_fq, zifa_tmm, zifa_tc, zifa_raw, pc_deseq, pc_tmm, pc_fq, pc_tc, pc_raw, file="allen_covariates.rda")
```
