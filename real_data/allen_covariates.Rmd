...
```{r zifa_cov}
df <- data.frame(Z1=zifa_tc[,1], Z2=zifa_tc[,2], coverage=coverage, detection_rate=detection_rate) 
...
cors <- sapply(1:2, function(i) cor(zifa_tc[,i], df[,3:4]))
colnames(cors) <- paste("PC", 1:NCOL(cors), sep="")
rownames(cors) <- paste("QC", 1:NROW(cors), sep="")
barplot(abs(cors), beside = TRUE, col=col1[1:2], border=col1[1:2], ylim=c(0, 1), space=c(0, 2), main="Absolute correlation with QC features")
legend("topright", colnames(df)[3:4], fill=col1, border=col1, cex=1)
```

## ZINB

```{r zinb_cov}
df <- data.frame(W1=zinb@W[,1], W2=zinb@W[,2], coverage=coverage, detection_rate=detection_rate) 
...
cors <- sapply(1:2, function(i) cor(zinb@W[,i], df[,3:4]))
colnames(cors) <- paste("PC", 1:NCOL(cors), sep="")
rownames(cors) <- paste("QC", 1:NROW(cors), sep="")
barplot(abs(cors), beside = TRUE, col=col1[1:2], border=col1[1:2], ylim=c(0, 1), space=c(0, 2), main="Absolute correlation with QC features")
legend("topright", colnames(df)[3:4], fill=col1, border=col1, cex=1)
