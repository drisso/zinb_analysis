...
```{r zinb_batch}
zinb_batch <- zinbFit(raw[vargenes,], K = 2, X=model.matrix(~Batch, data=targets), epsilon=1e3)
plot(zinb_batch@W, pch=19, col=colMerged, main="ZINB")
```

```{r zinb_qc,eval=FALSE}
library(scater)
...
zinb_qc <- zinbFit(raw[vargenes,], K = 2, X=model.matrix(~qcpca$x[,1:2]), epsilon=1e3)
...
zinb_gc <- zinbFit(raw[vargenes,], K = 2, 
...
## ComBat
```{r}
combat_raw <- ComBat(log1p(raw[vargenes,]), targets$Batch, BPPARAM = bpparam())
combat_tc  <- ComBat(log1p(tc[vargenes, ]), targets$Batch, BPPARAM = bpparam())
combat_tmm <- ComBat(log1p(tmm[vargenes,]), targets$Batch, BPPARAM = bpparam())
combat_fq  <- ComBat(log1p(fq[vargenes, ]), targets$Batch, BPPARAM = bpparam())

pc_combat_raw <- fastpca(combat_raw)
pc_combat_tc  <- fastpca(combat_tc)
pc_combat_tmm <- fastpca(combat_tmm)
pc_combat_fq  <- fastpca(combat_fq)
```


```{r write_out, dependson=c("zinb", "zifa", "pca", "zinb_batch", "zinb_qc"), eval=FALSE}
save(zinb, zinb_batch, zinb_qc, zifa_fq, zifa_tmm, zifa_tc, zifa_raw, pc_tmm, pc_fq, pc_tc, pc_raw, file="espresso_covariates.rda")
```

```{r write_out2, dependson=c("zinb", "zifa", "pca", "zinb_batch")}
save(zinb, zinb_batch, zifa_fq, zifa_tmm, zifa_tc, zifa_raw, pc_tmm, pc_fq, pc_tc, pc_raw, pc_combat_raw, pc_combat_fq, pc_combat_tc, pc_combat_tmm, file="espresso_covariates.rda")
```
