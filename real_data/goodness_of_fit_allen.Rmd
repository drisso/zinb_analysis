...
```{r zinb}
zinb <- zinbFit(core, K = 2, epsilon = 1e3, commondispersion = FALSE)
```
...
P_z_1 = invlogit(coef(fit, 'D') %*% t(X))
mean_gaussian = coef(fit, 'C') %*% t(X)
var_gaussian = fit@dispersion[,'C']
P_gaussian_0 = (1/sqrt(2*pi*var_gaussian))* exp(-0.5 * mean_gaussian^2 / var_gaussian)
hurdleEY_1 = rowMeans((P_z_1 / (1 - P_gaussian_0)) * mean_gaussian)
hurdlePY0_1 = rowMeans(1 - P_z_1)
```

```{r mastnocov}
sca = newSeqExpressionSet(core)
#fq = EDASeq::betweenLaneNormalization(sca, 'full')
sca = FromMatrix(log1p(counts(sca)))
...
var_gaussian = fit@dispersion[,'C']
P_gaussian_0 = (1/sqrt(2*pi*var_gaussian))* exp(-0.5 * mean_gaussian^2 / var_gaussian)
hurdleEY_2 = rowMeans((P_z_1 / (1 - P_gaussian_0)) * mean_gaussian)
hurdlePY0_2 = rowMeans(1 - P_z_1)
```
...
plotMD(hurdleEY_1, logAveCount, xlim = xlim, ylim = ylim, main = 'Hurdle, MAST, gene det as a cov')
plotMD(hurdleEY_2, logAveCount, xlim = xlim, ylim = ylim, main = 'Hurdle, MAST')
#dev.off()
```
...
plotMD(hurdlePY0_1, prop0, xlim = c(0, .8), ylim = c(-.2, .1), main = 'Hurdle, MAST, gene det as a cov')
plotMD(hurdlePY0_2, prop0, xlim = c(0, .8), ylim = c(-.2, .1), main = 'Hurdle, MAST')
#dev.off()
```
