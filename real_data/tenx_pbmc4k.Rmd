---
title: "4k PBMCs from 10x genomics"
author: "Davide Risso"
date: "07/10/2017"
output: 
  html_document: 
...
```{r}
save(zinb_res, file="tenx_pbmc4k/zinb_res.rda")
write.table(zinb_df, file="tenx_pbmc4k/zinb_coords.txt", sep='\t', quote=FALSE, 
            row.names = TRUE)
```
...
## Compute additional metrics (length, gcc)
library(biomaRt)
...
bm <- getBM(attributes=c('ensembl_gene_id', 'hgnc_symbol', 'start_position',
            ...
            values = rownames(pbmc)[use_genes],
            mart = mart)

type <- bm$gene_biotype
names(type) <- bm$ensembl_gene_id

## length
bm$length <- bm$end_position - bm$start_position
length <- bm$length
names(length) <- bm$ensembl_gene_id

pbmc_exp <- pbmc[use_genes,]
gene_idx <- intersect(names(length), rownames(pbmc_exp))
length(gene_idx)

pbmc_exp <- pbmc_exp[gene_idx,]
length <- length[gene_idx]
len <- colSums(exprs(pbmc_exp) * length)/colSums(exprs(pbmc_exp))

##gc content
gcc <- bm$percentage_gene_gc_content
names(gcc) <- bm$ensembl_gene_id

gcc <- gcc[gene_idx]
gc <- colSums(exprs(pbmc_exp) * gcc)/colSums(exprs(pbmc_exp))
...
save(pc_tc, file="tenx_pbmc4k/pca_res.rda")
```
...
save(zinb_gcc, file="tenx_pbmc4k/zinb_gcc.rda")
```
...
# More on GC-content and length

It seems that including gc-content and length in V helps, but why is that?

We can look at how the PCA loadings and the alpha in ZINB are related to gcc and length.

```{r}
stopifnot(all(rownames(bm)==rownames(dense)))

alpha_mu <- t(getAlpha_mu(zinb_res))
alpha_pi <- t(getAlpha_pi(zinb_res))

bins <- cut(bm$length, breaks = quantile(bm$length, probs=seq(0, 1, 0.05)))
plot(alpha_mu[,1]~bins, outline=FALSE)
plot(alpha_mu[,2]~bins, outline=FALSE)
plot(alpha_mu[,3]~bins, outline=FALSE)

plot(alpha_pi[,1]~bins, outline=FALSE)
plot(alpha_pi[,2]~bins, outline=FALSE)
plot(alpha_pi[,3]~bins, outline=FALSE)

bins <- cut(bm$percentage_gene_gc_content, 
            breaks = quantile(bm$percentage_gene_gc_content, probs=seq(0, 1, 0.05)))
plot(alpha_mu[,1]~bins, outline=FALSE)
plot(alpha_mu[,2]~bins, outline=FALSE)
plot(alpha_mu[,3]~bins, outline=FALSE)
...
There does not seem to be any strong effects.

We can check PCA loadings.

```{r}
rownames(tc) <- rownames(pbmc)[use_genes]
...
plot(pca$rotation[,2]~bins, outline=FALSE)
plot(pca$rotation[,3]~bins, outline=FALSE)
...
Let's look at the dependence of UMI counts on gene length.
...
## all genes
xx <- log1p(exprs(pbmc_exp)[exprs(pbmc_exp)[,1]>0,1])
...
## average expression
filter <- rowSums(exprs(pbmc_exp)>0)>=100
filtered <- exprs(pbmc_exp)[filter,]
