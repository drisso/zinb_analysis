...
library(scater)
library(magrittr)
sceset <- newSCESet(countData = exprs(t4k[use_genes, keep]))

sceset <- calculateQCMetrics(sceset)
qc <- pData(sceset)[,c(2, 5, 7, 12:15)]

ribo_idx <- grep("^RPL", fData(t4k)[,2])
mito_idx <- grep("^MT-", fData(t4k)[,2])
ribo_pct <- colSums(as.matrix(exprs(t4k[ribo_idx,])))/colSums(as.matrix(exprs(t4k))) * 100
mito_pct <- colSums(as.matrix(exprs(t4k[mito_idx,])))/colSums(as.matrix(exprs(t4k))) * 100

qc <- cbind(qc, pct_ribo = ribo_pct[keep], pct_mito = mito_pct[keep])
