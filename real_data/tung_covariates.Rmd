...
ercc_prop <- colSums(umi[grep("^ERCC", rownames(umi)),])/colSums(umi)
detection_rate <- colSums(dat>0)

qc <- data.frame(ercc_prop, nmolecule, well, concentration, detection_rate)
...
To speed up computations we consider only the top 1000 most variable genes.
...
plot(zifa_tc, pch=19, col=colorbatch[batch], main="ZIFA TC")

zifa_tmm <- wrapRzifa(log1p(tmm))
plot(zifa_tmm, pch=19, col=colorbatch[batch], main="ZIFA TMM")

zifa_deseq <- wrapRzifa(log1p(deseq))
plot(zifa_deseq, pch=19, col=colorbatch[batch], main="ZIFA DESEQ")

zifa_fq <- wrapRzifa(log1p(fq))
plot(zifa_fq, pch=19, col=colorbatch[batch], main="ZIFA FQ")
```
...
plot(zinb@W, pch=19, col=colorbatch[batch], main="ZINB")
```
