...
for (nc in c(100)){
  for (offs in c(-3.5, 0, 3.5)){
    for (aa in c(1, .85)){
      ...
      corr <- lapply(1:13, function(i) rowMeans(sapply(res, function(x) x[[i]])))
      ...
      boxplot(corr, main = main, border = c(3, 1, 2, rep(1,10)), xlab = "",
  ...
  for (offs in c(-3.5, 0)){
    for (aa in c(1, .85)){
      pref = sprintf('datasets/sim%s_%s_a%s_offs%s_seed9128',
                     ds, nc, aa, offs)
      load(paste0(pref, '_dist.rda'))
      sil <- lapply(14:length(res[[1]]), function(i){
      ...
      boxplot(sil - sil[, 1], main = paste0("Difference of ", main),
              border = c(3, 1, 2, rep(1,10)), xlab = "", las = 2,
