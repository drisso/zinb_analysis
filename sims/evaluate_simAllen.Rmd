...
```{r corr25}
load("./datasets/data/data/simAllen_100_a0.85_offs-3.5_seed9128_dist.rda")
...
colnames(corr25) <- c("true W", paste0("zinb k=", 1:4), 
                      ...
                      "PCA FQ (k=2)", 'ZIFA (k=2)', 'ZIFA TC (k=2)', 
                      'ZIFA TMM (k=2)', 'ZIFA FQ (k=2)')
corr25$pzero = .25
boxplot(corr25[,1:13], main="Correlation between true and estimated sample distances in W\npzero = 0.25", border=c(3, 1, 2, rep(1,10)), xlab="",
        ylab="Correlation", las=2)
abline(h=1,lty = 2)
```

```{r corr45}
load("./datasets/data/data/simAllen_100_a0.85_offs0_seed9128_dist.rda")
corr45 <- lapply(1:13, function(i) rowMeans(sapply(res, function(x) x[[i]])))
...
colnames(corr45) <- c("true W", paste0("zinb k=", 1:4), 
                      ...
                      'ZIFA TMM (k=2)', 'ZIFA FQ (k=2)')
corr45$pzero = .45
boxplot(corr45[,1:13], main="Correlation between true and estimated sample distances in W\npzero = 0.45", border=c(3, 1, 2, rep(1,10)), xlab="method", ylab="Correlation", las=2)
abline(h=1,lty = 2)
```
...
corr = lapply(1:2, function(x){
  lapply(c(25,45,75), function(y){
    nn = sprintf("./datasets/simAllen_var%s_z%s_dist.rda", x, y)
    load(nn)
