...
sil = lapply(c(100, 1000, 10000), function(nc){
  ...
  lapply(c('', '_ziadd0.33', '_ziadd0.67'), function(zf){
    ...
    ss['zfrac'] = ifelse(zfrac == '', 40,
                         ifelse(zfrac == '0.33', 60, 80))
    ss['nc'] = nc
    ss
  })
})
...
colnames(silDF) <- c("ZINB-WaVE", "PCA_RAW","PCA_TC", "PCA_TMM", "PCA_FQ", "ZIFA_RAW", "ZIFA_TC", "ZIFA_TMM", "ZIFA_FQ", 'zfrac', 'nc')
...
eval_pr <- function(labels, dm, metrics = c('precision', 'recall')) {
  labels = as.integer(labels)
  km = kmeans(dm, centers = 3, iter.max = 1000, nstart = 1000)
  estClust = km$cluster
  pr = extCriteria(labels, estClust, metrics)  
  return(unlist(pr))
}

fastpca <- function(expr, scale=FALSE) {
  svd_raw <- svds(scale(expr, center=TRUE, scale=scale), k=2, nu=2, nv=0)
  pc_raw <- svd_raw$u %*% diag(svd_raw$d[1:2])
  return(pc_raw)
}
...
eval_data <- function(pp, counts, labels, fittedSim, k = 1:4, sim = 1) {
  ...
  ## ZINB
  pr = lapply(k, function(i){
    fit = fittedSim[[i]][[sim]]
    eval_pr(labels, fit@W)
  })

  ## PCA
  m = length(pr) + 1
  pca <- fastpca(log1p(t(counts)))
  pr[[m]] <- eval_pr(labels, pca)
  ...
  fact = colSums(counts)
  tc = mult * (t(counts) / fact)
  pcatc <- fastpca(log1p(tc))
  pr[[m]] <- eval_pr(labels, pcatc)
  ...
  y = calcNormFactors(y, method="TMM")
  tmm <- t(counts) / (y$samples$lib.size * y$samples$norm.factors)
  pcatmm <- fastpca(log1p(tmm))
  pr[[m]] <- eval_pr(labels, pcatmm)
  
  ## PCA FQ
  m = m + 1
  fq <- betweenLaneNormalization(counts, which="full")
  pcafq <- fastpca(t(log1p(fq)))
  pr[[m]] <- eval_pr(labels, pcafq)
  ...
  if (ZIFA){
    ...
    ## ZIFA
    m = m + 1
    pr[[m]] <- eval_pr(labels, zifa[[sim]])
    
    ## ZIFA TC
    m = m + 1
    pr[[m]] <- eval_pr(labels, zifaTC[[sim]])
    
    ## ZIFA TMM
    m = m + 1
    pr[[m]] <- eval_pr(labels, zifaTMM[[sim]])
    
    ## ZIFA FQ
    m = m + 1
    pr[[m]] <- eval_pr(labels, zifaFQ[[sim]])
  }
  pr = do.call(rbind, pr)
  return(pr)
}

pr = lapply(c(100, 1000, 10000), function(nc){
  print(nc)
  lapply(c('', '_ziadd0.33', '_ziadd0.67'), function(zf){
    print(zf)
    pp = sprintf('fig6e-g/simLun_%s%s', nc, zf)
    ...
    res <- lapply(1:10, function(j){
      ...
      if (nc == 10000){
        eval_data(pp, counts, labels, fittedSim, sim = j, k = 1)
      }else{
        eval_data(pp, counts, labels, fittedSim, sim = j, k = 2) 
      }
    })
    ss = Reduce("+", res) / length(res)
    rownames(ss) = c("ZINB-WaVE", "PCA_RAW","PCA_TC", "PCA_TMM",
                     "PCA_FQ", "ZIFA_RAW", "ZIFA_TC", "ZIFA_TMM",
                     "ZIFA_FQ")
    ss = melt(ss)
    zfrac = gsub('_ziadd', '', zf)
    ss[,'zfrac'] = ifelse(zfrac == '', 40,
...
prDF = do.call(rbind, do.call(rbind, pr))
prDF = data.frame(prDF, stringsAsFactors = F)
prDF$zfrac = factor(prDF$zfrac)
prDF$nc = paste0('ncells = ', prDF$nc)
#prMolt = melt(prDF, id.vars = c('zfrac', 'nc'))
write.csv(prDF, file = '../figures/fig6e-g/prDF.csv')
prDF = read.csv('../figures/fig6e-g/prDF.csv', stringsAsFactors = FALSE)
prDF$zfrac = factor(prDF$zfrac)
prDF$X1[prDF$X1 == 'ZINB-WaVE'] = "ZINB-WaVE_K=2"
prDF$X1 = factor(prDF$X1, levels = c("ZINB-WaVE_K=2", "PCA_RAW",
                 "PCA_TC", "PCA_TMM", "PCA_FQ", "ZIFA_RAW", "ZIFA_TC", 
                 "ZIFA_TMM", "ZIFA_FQ"))
pLun = ggplot(prDF[prDF$X2 == 'precision', ],
  aes(x = zfrac, y =value, col = X1, group = X1)) +
  geom_point() + geom_line() + labs(col='') + 
...
rLun = ggplot(prDF[prDF$X2 == 'recall', ],
  aes(x = zfrac, y =value, col = X1, group = X1)) +
  geom_point() + geom_line() + labs(col='') + 
  theme_bw() + xlab('Zero fraction') + facet_grid( ~ nc) +
  ylab('Recall') +
...
prLun = plot_grid(pLun + theme(legend.position="none"),
                  rLun + theme(legend.position="none"),
                  align = "h", labels = c("a", "b"), nrow = 2, ncol = 1)

legend <- get_legend(pLun)
prLun <- plot_grid(prLun, legend, rel_widths = c(3, .6))

# save figure suppl of figure 6 panels e-g
ggsave(filename="../../paper/6680489mtyrjx/precisionRecallLun.pdf",
       plot = prLun,
       width = 8, height = 5)
```
